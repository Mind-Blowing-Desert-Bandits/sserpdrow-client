...
var config = {
  apiOrigins: {
    production: 'https://sserpdrow-mbdb-api.herokuapp.com',
...
var sitesEvents = __webpack_require__(9);
var reuse = __webpack_require__(11);
...
var signInSuccess = function signInSuccess(data) {
  reuse.hideMultipleFields(['#sign-up', '#signInAfterSignUp', '#sign-in', '#signUpLink', '#signUpModal', '#get-sites', '#mySite', '#myPages', '#myBlogs', '#mySiteHeader']);
...
var signInNewUserSuccess = function signInNewUserSuccess(data) {
  reuse.hideMultipleFields(['#sign-up', '#signInAfterSignUp', '#sign-in', '#signUpLink', '#signUpModal', '#signInMessage', '#userSignedOut', '#userSignedOut', '#get-sites']);
...
var signOutSuccess = function signOutSuccess() {
  reuse.hideMultipleFields(['#changePassword', '#signOut', '#signUpModal', '#signedIn', '#editASite', '#manageBlogSection', '#newBlog', '#managePagesSection', '#newPage', '#editPageSection', '#editBlogSection', '#userDashboard', '#createASite', '#returnToSite', '#dashboardLink', '#mySite']);
  reuse.showMultipleFields(['#signUpLink', '#messageContent', '#signInMessage', '#signInModal', '#get-sites', '#userSignedOut']);
  reuse.emptyMultipleTextFields(['#messageContent', '#signInMessage', '#userNameNav', '#blogs', '#allSites', '#sitePages']);
  store.user = null;
  store.site = null;
  // $('#changePassword').hide()
  // $('#signOut').hide()
...
module.exports = {
  ...
  signInNewUserSuccess: signInNewUserSuccess
};
...
var promiseGetSite = function promiseGetSite() {
  return api.getSite(store.site.id);
};

var onGetSites = function onGetSites(event) {
  event.preventDefault();
  $('#allSites').text('');
  $('#blogs').hide();
  $('#returnToSite').hide();
  $('#sitePages').text('');
  api.getSites().then(ui.getSitesSuccess).then(function () {
    $('.view').on('click', viewSite);
  }).catch(ui.getSitesFailure);
};

var viewSite = function viewSite(event) {
  event.preventDefault();
  var site = event.target;
  var siteId = site.parentNode.parentNode;
  var thisSiteID = siteId.getAttribute('data-id');
  api.getSite(thisSiteID).then(ui.viewSiteSuccess).then(function () {
    $('.show-pages').on('click', viewPage);
  }).catch(ui.viewSiteFailure);
};

var createSite = function createSite(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.createSite(data).then(ui.createSiteSuccess).catch(ui.createSiteFailure);
};

var viewPage = function viewPage(event) {
  event.preventDefault();
  var page = event.target;
  var pageId = page.parentNode;
  var pageParent = pageId.parentNode;
  var thisID = pageParent.getAttribute('data-id');
  for (var i = 0; i < store.site.site.pages.length; i++) {
...
var newBlogPost = function newBlogPost(event) {
  event.preventDefault();
  var data = getFormFields(this);
  var newBlog = {
  ...
  api.addBlogPost(store.site.blogposts).then(ui.addBlogPostSuccess).then(promiseGetSite).then(ui.updateLocalSiteVar).then(manageBlog).catch(ui.addBlogPostFailure);
};

var newPage = function newPage(event) {
  event.preventDefault();
  var data = getFormFields(this);
  var newPageObject = {
  ...
  api.addPage(store.site.pages).then(ui.addPageSuccess).then(api.getSites).then(ui.updateLocalSitePageVar).then(managePages).catch(ui.addPageFailure);
};

var getUpdatedSiteByUser = function getUpdatedSiteByUser() {
  api.getSites().then(ui.getUpdatedSiteSuccess).catch(ui.getSitesFailure);
};

var showMySite = function showMySite(event) {
  event.preventDefault();
  api.getMySite().then(ui.showMySiteSuccess).then(function () {
    $('#myPages').on('click', viewMyPage);
  }).catch(ui.showMySiteFailure);
};

var viewMyPage = function viewMyPage(event) {
  event.preventDefault();
  var page = event.target;
  var pageId = page.parentNode;
  var pageParent = pageId.parentNode;
  var thisID = pageParent.getAttribute('data-id');
  for (var i = 0; i < store.mySite.site.pages.length; i++) {
...
var manageBlog = function manageBlog() {
  // Hide dashboard
  $('#userDashboard').hide();
  ui.manageBlog();
  $('.editBlog').on('click', function (event) {
    var div = $(this).parents()[4];
...
var dataIdFilter = function dataIdFilter(dataId) {
  var blogpost = store.site.blogposts.filter(function (blog) {
...
var dataPageIdFilter = function dataPageIdFilter(dataId) {
  var page = store.site.pages.filter(function (page) {
...
var showCreateBlogForm = function showCreateBlogForm() {
  $('#newBlog').show();
  $('#manageBlogSection').hide();
};

var cancelNewBlog = function cancelNewBlog() {
  $('#newBlog').hide();
  $('#manageBlogSection').show();
  document.getElementById('newBlogForm').reset();
};

var cancelEditBlog = function cancelEditBlog() {
  $('#editBlogSection').hide();
  $('#manageBlogSection').show();
  document.getElementById('editBlogForm').reset();
};

var cancelEditPage = function cancelEditPage() {
  $('#editPageSection').hide();
  $('#managePagesSection').show();
  document.getElementById('editPageForm').reset();
};

var showEditBlogForm = function showEditBlogForm() {
  $('#manageBlogSection').hide();
  $('#editBlogSection').show();
};

var editBlogContent = function editBlogContent(event) {
  event.preventDefault();
  var data = getFormFields(this);
  store.site.blogposts.forEach(function (blog, index) {
  ...
  api.addBlogPost(store.site.blogposts).then(ui.editBlogPostSuccess).then(promiseGetSite).then(ui.updateLocalSiteVar).then(manageBlog).catch(ui.editBlogPostFailure);
};

var deleteBlog = function deleteBlog(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.deleteBlogPost(data.blogposts.id).then(promiseGetSite).then(ui.updateLocalSiteVar).then(manageBlog).then(ui.deleteBlogPostSuccess).catch(ui.deleteBlogPostFailure);
};
...
var showEditSiteForm = function showEditSiteForm() {
  $('#editASite').show();
  $('#userDashboard').hide();
...
var cancelEditSite = function cancelEditSite() {
  $('#editASite').hide();
  $('#userDashboard').show();
  document.getElementById('editSiteForm').reset();
};

var editSite = function editSite(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.editSite(data).then(api.getMySite).then(ui.editSiteSuccess).catch(ui.editSiteFailure);
};

var showDeleteSiteModal = function showDeleteSiteModal() {
  $('#deleteSiteModal').modal('show');
};

var closeDeleteSiteModal = function closeDeleteSiteModal() {
  $('#deleteSiteModal').modal('hide');
};

var deleteSite = function deleteSite(event) {
  event.preventDefault();
  api.deleteSite().then(ui.deleteSiteSuccess).catch(ui.deleteSiteFailure);
};

var managePages = function managePages() {
  // Hide dashboard
  $('#userDashboard').hide();
  ui.managePages();
...
var showCreatePageForm = function showCreatePageForm() {
  $('#newPage').show();
  $('#managePagesSection').hide();
};

var showEditPageForm = function showEditPageForm() {
  $('#managePagesSection').hide();
  $('#editPageSection').show();
};

var editPageContent = function editPageContent(event) {
  event.preventDefault();
  var data = getFormFields(this);
  store.site.pages.forEach(function (page, index) {
  ...
  api.addPage(store.site.pages).then(ui.editPageSuccess).then(api.getSites).then(ui.updateLocalSitePageVar).then(managePages).catch(ui.editPageFailure);
};

var deletePage = function deletePage(event) {
  event.preventDefault();
  var data = getFormFields(this);
  api.deletePageAPI(data.pages.id).then(api.getSites).then(ui.updateLocalSitePageVar).then(managePages).then(ui.deletePageSuccess).catch(ui.deletePageFailure);
};

var returnToSite = function returnToSite(event) {
  event.preventDefault();
  ui.viewSiteSuccess(store.site);
};

var returnToMySite = function returnToMySite(event) {
  event.preventDefault();
...
var siteHandlers = function siteHandlers() {
...
module.exports = {
  ...
  getUpdatedSiteByUser: getUpdatedSiteByUser,
...
var removeDisabledClassesNavArray = function removeDisabledClassesNavArray(stringArray) {
  stringArray.forEach(function (id) {
    $(id).removeClass('disabled');
  });
};

var addDisableClassNavArray = function addDisableClassNavArray(stringArray) {
  stringArray.forEach(function (id) {
    $(id).addClass('disabled');
  });
};

var removeActiveClassesNavArray = function removeActiveClassesNavArray(stringArray) {
  stringArray.forEach(function (id) {
    $(id).removeClass('active');
  });
};

var hideMultipleFields = function hideMultipleFields(stringArray) {
  stringArray.forEach(function (id) {
    $(id).hide();
  });
};

var showMultipleFields = function showMultipleFields(stringArray) {
  stringArray.forEach(function (id) {
    $(id).show();
  });
};

var emptyMultipleTextFields = function emptyMultipleTextFields(stringArray) {
  stringArray.forEach(function (id) {
    $(id).text('');
  });
};

var removeValMultipleTextFields = function removeValMultipleTextFields(stringArray) {
  stringArray.forEach(function (id) {
    $(id).val('');
  });
};

var updateFieldAddRemoveClassMessage = function updateFieldAddRemoveClassMessage(fieldIDStr, newTextStr, addClassStr, removeClassStr) {
...
var arrayComparison = function arrayComparison(Array1, Array2) {
  return Array1.filter(function (val) {
    return val.indexOf(Array2) < 0;
  });
};
...
module.exports = {
  removeDisabledClassesNavArray: removeDisabledClassesNavArray,
  addDisableClassNavArray: addDisableClassNavArray,
  removeActiveClassesNavArray: removeActiveClassesNavArray,
  hideMultipleFields: hideMultipleFields,
  emptyMultipleTextFields: emptyMultipleTextFields,
  updateFieldAddRemoveClassMessage: updateFieldAddRemoveClassMessage,
  showMultipleFields: showMultipleFields,
  removeValMultipleTextFields: removeValMultipleTextFields,
  arrayComparison: arrayComparison,
  shuffle: shuffle,
  replaceNullUndefined: replaceNullUndefined
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var siteEvents = __webpack_require__(9);
...
var onSignUp = function onSignUp(event) {
  ...
  if (data.credentials.password !== data.credentials.password_confirmation) {
    ...
    }).then(ui.signInNewUserSuccess).catch(ui.signUpFailure);
  }
};
...
var getSite = function getSite(thisID) {
  return $.ajax({
...
var addBlogPost = function addBlogPost(data) {
  var updatedata = {
    'site': {
      'blogposts': data
    }
...
var deleteBlogPost = function deleteBlogPost(data) {
  var updatedata = {
    'site': {
      'blogID': data
    }
...
var addPage = function addPage(data) {
  var updatedata = {
    'site': {
      'pages': data
    }
...
var deletePageAPI = function deletePageAPI(data) {
  var updatedata = {
    'site': {
      'pageID': data
    }
...
var getMySite = function getMySite() {
  return $.ajax({
    url: config.apiOrigin + '/sites/' + store.site.id,
    method: 'GET',
    headers: {
...
var editSite = function editSite(data) {
  return $.ajax({
    url: config.apiOrigin + '/sites/' + store.site.id,
    method: 'PATCH',
    headers: {
...
var deleteSite = function deleteSite() {
  return $.ajax({
    url: config.apiOrigin + '/sites/' + store.site.id,
    method: 'DELETE',
    headers: {
...
var store = __webpack_require__(3);
var showSitesTemplate = __webpack_require__(21);
var showSiteTemplate = __webpack_require__(38);
var showPagesTemplate = __webpack_require__(39);
var showPageSuccessTemplate = __webpack_require__(40);
var showMySiteTemplate = __webpack_require__(41);
var showBlogsTemplate = __webpack_require__(42);
var showMyPagesTemplate = __webpack_require__(43);

var getSitesSuccess = function getSitesSuccess(sites) {
  store.sites = sites;
  var showSites = showSitesTemplate({
    sites: sites.sites
  });
...
var getUpdatedSiteSuccess = function getUpdatedSiteSuccess(data) {
  var userSites = data.sites.filter(function (site) {
...
var getSitesFailure = function getSitesFailure() {
  $('#allSites').text('Please try again');
};

var viewSiteSuccess = function viewSiteSuccess(site) {
  store.site = site;
  var showSite = showSiteTemplate({ blogs: site.site.blogposts });
  var showPages = showPagesTemplate({ pages: site.site.pages });
  $('#returnToSite').hide();
  $('#allSites').text('');
...
var viewSiteFailure = function viewSiteFailure() {
  $('#allSites').text('Please try again');
};

var showPageSuccess = function showPageSuccess() {
  $('#blogs').hide();
  $('#returnToSite').show();
  var page = store.page;
  var showPage = showPageSuccessTemplate({ page: page });
  $('#allSites').text('');
  $('#allSites').append(showPage);
};

var showMyPageSuccess = function showMyPageSuccess() {
  $('#blogs').hide();
  var page = store.myPage;
  var showPage = showPageSuccessTemplate({ page: page });
  $('#myBlogs').text('');
...
var createSiteSuccess = function createSiteSuccess(data) {
...
var createSiteFailure = function createSiteFailure() {
...
var addBlogPostSuccess = function addBlogPostSuccess() {
...
var addBlogPostFailure = function addBlogPostFailure() {
...
var addPageSuccess = function addPageSuccess() {
  return new Promise(function (resolve, reject) {
    document.getElementById('newPageForm').reset();
    // hiding the form and bringing back updated blogs page
    $('#newPage').hide();
...
var addPageFailure = function addPageFailure() {
  $('#createAPageMessage').text('Unexpected Error. Please try again.');
};

var showMySiteSuccess = function showMySiteSuccess(data) {
  store.mySite = data;
  var site = store.mySite;
  var pages = site.site.pages;
  var blogs = site.site.blogposts;
  var mySite = showMySiteTemplate({ site: site });
  var myPages = showPagesTemplate({ pages: pages });
  var myBlogs = showSiteTemplate({ blogs: blogs });
  $('#returnToMySite').hide();
  $('#userDashboard').hide();
  $('#newBlog').hide();
  $('#newPage').hide();
...
var editPageSuccess = function editPageSuccess() {
  return new Promise(function (resolve, reject) {
    document.getElementById('editPageForm').reset();
    // hiding the form and bringing back updated blogs page
    $('#editPageSection').hide();
...
var editPageFailure = function editPageFailure() {
...
var editBlogPostSuccess = function editBlogPostSuccess() {
  return new Promise(function (resolve, reject) {
    document.getElementById('editBlogForm').reset();
    // hiding the form and bringing back updated blogs page
    $('#editABlogMessage').text('');
...
var editBlogPostFailure = function editBlogPostFailure() {
  $('#editABlogMessage').text('Unexpected Error. Please try again.');
};

var manageBlog = function manageBlog() {
  var showBlogs = showBlogsTemplate({
...
var updateLocalSiteVar = function updateLocalSiteVar(data) {
  return new Promise(function (resolve, reject) {
    store.site = data.site;
    resolve();
  });
...
var updateLocalSitePageVar = function updateLocalSitePageVar(data) {
  return new Promise(function (resolve, reject) {
    var userSites = data.sites.filter(function (site) {
      return site['_owner'] === store.user['_id'];
    });
...
var deleteBlogPostSuccess = function deleteBlogPostSuccess() {
  $('#deleteModal').modal('hide');
  $('#deleteABlogMessage').text('');
};
var deleteBlogPostFailure = function deleteBlogPostFailure() {
  $('#deleteABlogMessage').text('Unexpected Error. Please try again.');
};

var editSiteSuccess = function editSiteSuccess(data) {
  document.getElementById('editSiteForm').reset();
  store.site = data.site;
...
var editSiteFailure = function editSiteFailure() {
  $('#edit-site-failure').text('Unexpected Error. Please try again.');
};

var deleteSiteSuccess = function deleteSiteSuccess() {
  store.site = null;
  $('#deleteSiteModal').modal('hide');
  $('#userDashboard').hide();
  $('#dashboardLink').hide();
...
var deleteSiteFailure = function deleteSiteFailure() {
  $('#delete-site-failure').text('Unexpected Error. Please try again.');
};

var managePages = function managePages() {
  var showMyPages = showMyPagesTemplate({ pages: store.site.pages });
  $('#mPage').html(showMyPages);
  $('#pageTitle').text(store.site.title);
  $('#pageDescription').text(store.site.description);
};

var deletePageSuccess = function deletePageSuccess() {
  $('#deletePageModal').modal('hide');
  $('#delete-page-failure').text('');
};

var deletePageFailure = function deletePageFailure() {
  $('#delete-page-failure').text('Unexpected Error. Please try again.');
};

module.exports = {
  getSitesSuccess: getSitesSuccess,
  getSitesFailure: getSitesFailure,
  viewSiteSuccess: viewSiteSuccess,
  viewSiteFailure: viewSiteFailure,
  showPageSuccess: showPageSuccess,
  createSiteSuccess: createSiteSuccess,
  createSiteFailure: createSiteFailure,
  getUpdatedSiteSuccess: getUpdatedSiteSuccess,
  addBlogPostSuccess: addBlogPostSuccess,
  addBlogPostFailure: addBlogPostFailure,
  showMySiteSuccess: showMySiteSuccess,
  showMyPageSuccess: showMyPageSuccess,
  manageBlog: manageBlog,
  updateLocalSiteVar: updateLocalSiteVar,
  editBlogPostSuccess: editBlogPostSuccess,
  editSiteSuccess: editSiteSuccess,
  deleteSiteSuccess: deleteSiteSuccess,
  editBlogPostFailure: editBlogPostFailure,
  editPageSuccess: editPageSuccess,
  deleteBlogPostSuccess: deleteBlogPostSuccess,
  deleteBlogPostFailure: deleteBlogPostFailure,
  managePages: managePages,
  addPageSuccess: addPageSuccess,
  deletePageSuccess: deletePageSuccess,
  updateLocalSitePageVar: updateLocalSitePageVar,
  editSiteFailure: editSiteFailure,
  deleteSiteFailure: deleteSiteFailure,
  editPageFailure: editPageFailure,
  deletePageFailure: deletePageFailure,
  addPageFailure: addPageFailure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<br>\n  <div class=\"panel panel-info\" data-id=\""
    + container.escapeExpression(container.lambda(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n"
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.title : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.description : stack1),{"name":"if","hash":{},"fn":container.program(6, data, 0, blockParams),"inverse":container.program(8, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "      <div class=\"panel-footer\">\n        <button class=\"view btn btn-primary\">View Site</button>\n      </div>\n    </div>\n  </div>\n";
  ...
  return "      <div class=\"panel-heading\">"
    ...
    return "      <div class=\"panel-heading\">No Title</div>\n";
  ...
  return "        <div class=\"panel-body\">"
    ...
    return "        <div class=\"panel-body\">No Description</div>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "\n<!-- New  -->\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.sites : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<br>\n  <div class=\"panel panel-info\" data-id=\""
    ...
    + "\">\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.title : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "      <div class=\"panel-body\">\n        "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.textcontent : stack1), depth0))
    + "\n      </div>\n    </div>\n  </div>\n";
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;
  ...
  return "      <div class=\"panel-heading\">"
    ...
    + " - "
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.date : stack1), depth0))
    + "</div>\n";
},"4":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "      <div class=\"panel-heading\">No Title - "
  ...
  return "<!-- New  -->\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.blogs : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n <!-- Developers, why are we using data-id and not just id? -->\n<div class=\"pages\">\n  <p data-id=\""
    + container.escapeExpression(container.lambda(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n    <a href=\"#\">\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.type : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</a>\n    <br>\n  </p>\n\n</div>\n";
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "    <medium>"
    + container.escapeExpression(container.lambda(((stack1 = blockParams[1][0]) != null ? stack1.type : stack1), depth0))
    + " | </medium>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "    <b>No Page</b>\n    ";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return " <!-- New  -->\n <br>\n   <div class=\"panel panel-info\" data-id=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.page : depth0)) != null ? stack1.id : stack1), depth0))
    + "\">\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = (depth0 != null ? depth0.page : depth0)) != null ? stack1.title : stack1),{"name":"if","hash":{},"fn":container.program(1, data, 0),"inverse":container.program(3, data, 0),"data":data})) != null ? stack1 : "")
    + "       <div class=\"panel-body\">\n         "
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.page : depth0)) != null ? stack1.textcontent : stack1), depth0))
    + "\n       </div>\n     </div>\n   </div>\n";
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "      <h1>"
    + container.escapeExpression(container.lambda(((stack1 = ((stack1 = (depth0 != null ? depth0.site : depth0)) != null ? stack1.site : stack1)) != null ? stack1.title : stack1), depth0))
    + "</h1>\n";
},"3":function(container,depth0,helpers,partials,data) {
  ...
  return "      <p>\n        "
    + container.escapeExpression(container.lambda(((stack1 = ((stack1 = (depth0 != null ? depth0.site : depth0)) != null ? stack1.site : stack1)) != null ? stack1.description : stack1), depth0))
    + "\n      </p>\n";
  ...
  return "<!-- New  -->\n<br>\n  <div data-id=\""
    + container.escapeExpression(container.lambda(((stack1 = ((stack1 = (depth0 != null ? depth0.site : depth0)) != null ? stack1.site : stack1)) != null ? stack1.id : stack1), depth0))
    + "\">\n"
    + ((stack1 = helpers["if"].call(alias1,((stack1 = ((stack1 = (depth0 != null ? depth0.site : depth0)) != null ? stack1.site : stack1)) != null ? stack1.title : stack1),{"name":"if","hash":{},"fn":container.program(1, data, 0),"inverse":container.program(3, data, 0),"data":data})) != null ? stack1 : "")
    + ((stack1 = helpers["if"].call(alias1,((stack1 = ((stack1 = (depth0 != null ? depth0.site : depth0)) != null ? stack1.site : stack1)) != null ? stack1.description : stack1),{"name":"if","hash":{},"fn":container.program(5, data, 0),"inverse":container.program(7, data, 0),"data":data})) != null ? stack1 : "")
    + "    </div>\n  </div>\n";
},"useData":true});
